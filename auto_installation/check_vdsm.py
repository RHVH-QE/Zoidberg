...
from const_vdsm import RHVM_INFO, MACHINE_INFO, NFS_INFO, DELL_PER515_01
...
class CheckVdsm(CheckYoo):
    ...
    def __init__(self):
        ...
        self._host_info = {}  # {"host_ip": host_ip, "host_pass": host_pass, "host_name": vdsm_fc_host}
    ...
    def _get_network_ip(self):
        ...
        ret = self.run_cmd(cmd, timeout=300)
        if not ret[0]:
            raise RuntimeError("Failed to get the ip address of device %s" % device)
        return ret[1]
    ...
    def cl1_create_local_sd_check(self):
        ...
        try:
            ...
            if not storage_type == "localfs":
                raise RuntimeError("Storage type is %s, not localfs" % storage_type)
    ...
    def _get_network_info(self):
        if re.search("bondi", self.ksfile):
            if not MACHINE_INFO[self.beaker_name].get("bond", None):
                raise RuntimeError("%s not support bond test" % self.beaker_name)
            slaves = MACHINE_INFO[self.beaker_name]["bond"]["slaves"]
            self._network_info.update({"network_mode": "bond", "slaves": slaves})

        if re.search("vlani", self.ksfile):
            if not MACHINE_INFO[self.beaker_name].get("vlan", None):
                raise RuntimeError("%s not support vlan test" % self.beaker_name)
...
GATEWAY="{gateway}"\
    ...
    def _set_ifcfg_bond(self):
        ...
        for dev, ifcfg_str in ifcfg_str_dict.items():
            ...
            self.put_remote_file(tcfg, dcfg)

        # Restart the network
        cmd = "service network restart"
        ret = self.run_cmd(cmd, timeout=150)
        if not ret[0]:
    ...
    def run_cases(self):
        ...
        try:
            ...
            disorder_checkpoint_cases_map = get_checkpoint_cases_map(
