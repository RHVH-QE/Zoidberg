...
class VdsmInfo(object):
    ...
    def _get_rhvm_info(self):
        ...
        self._get_rhvm_fqdn()
        from ..helpers import RhevmAction
    ...
    def _get_host_vlanid(self):
        if re.search("vlan|bv", self._ksfile):
            if MACHINE_INFO[self._beaker_name].get('vlan', None):
                vlan_id = MACHINE_INFO[self._beaker_name]['vlan']['id']
            else:
                if MACHINE_INFO[self._beaker_name].get('bv', None):
                    vlan_id = MACHINE_INFO[self._beaker_name]['bv']['vlan_id']
    ...
    def _get_dc_info(self):
        scen = self._ksfile.split('_')[1]
        dc_name = datacenter_name_tpl.replace('tpl', scen)
        is_local = False
        if re.search('local', self._ksfile):
            is_local = True
    ...
    def _get_cluster_info(self):
        ...
        ret = self._remotecmd.run_cmd(cmd, timeout=300)
        if ret[0]:
            if AMD_CPU in ret[1]:
                cpu_type = "AMD Opteron G1"
    ...
    def _get_network_info(self):
        if re.search("bondi", self._ksfile):
            if not MACHINE_INFO[self._beaker_name].get("bond", None):
                raise RuntimeError("%s not support bond test" %
                                   self._beaker_name)
            slaves = MACHINE_INFO[self._beaker_name]["bond"]["slaves"]
            self.network_info.update(
                {"network_mode": "bond", "slaves": slaves})

        if re.search("vlani", self._ksfile):
            if not MACHINE_INFO[self._beaker_name].get("vlan", None):
                raise RuntimeError("%s not support vlan test" %
                                   self._beaker_name)
            id = MACHINE_INFO[self._beaker_name]["vlan"]["id"]
            nics = MACHINE_INFO[self._beaker_name]["vlan"]["nics"]
            static_ip = MACHINE_INFO[self._beaker_name]["vlan"]["static_ip"]
            self.network_info.update({
                "network_mode": "vlan", "id": id, "nics": nics, "static_ip": static_ip})

        if re.search("bvi", self._ksfile):
            if not MACHINE_INFO[self._beaker_name].get("bv", None):
                raise RuntimeError("%s not support bv test" % self._beaker_name)
            bond_name = MACHINE_INFO[self._beaker_name]["bv"]["bond_name"]
            vlan_id = MACHINE_INFO[self._beaker_name]["bv"]["vlan_id"]
            slaves = MACHINE_INFO[self._beaker_name]["bv"]["slaves"]
            static_ip = MACHINE_INFO[self._beaker_name]["bv"]["static_ip"]
            self.network_info.update({
                "network_mode": "bv",
                "bond_name": bond_name,
                "vlan_id": vlan_id,
    ...
    def _get_ifcfg_str(self, network_mode):
        ...
        if network_mode == "bond":
            ...
            cmd = "ip a s|grep %s" % self._remotecmd.host_string
            ...
            if bootproto == "dhcp":
                ...
                """.format(
                    primary=primary_nic,
                    device=bond_name,
                    ipaddr=self._remotecmd.host_string,
                    prefix=prefix,
                    gateway=gateway)
    ...
    def _set_ifcfg_bond(self):
        ...
        ret = self._remotecmd.run_cmd(cmd, timeout=150)
        if not ret[0]:
    ...
    def _setup_ifcfg_network_ip(self):
        ...
        elif re.search("vlani", self._ksfile):
    ...
    def _get_host_ip(self):
        if re.search("bondi|vlani|bvi", self._ksfile):
            host_ip = self._setup_ifcfg_network_ip()
        elif re.search("bonda|vlana|bva", self._ksfile):
            host_ip = self._get_network_ip()
        else:
            host_ip = self._remotecmd.host_string

        return host_ip

    def _get_host_pass(self):
        host_pass = MACHINE_INFO[self._beaker_name]['password']
        return host_pass
    ...
    def _get_storage_info(self):
        ...
        if re.search("local", self._ksfile):
            data_path = MACHINE_INFO[self._beaker_name]["local"]["data_path"]
        ...
        elif re.search("fc", self._ksfile):
            boot_lun = MACHINE_INFO[self._beaker_name]["fc"]["boot_lun"]
            avl_luns = MACHINE_INFO[self._beaker_name]["fc"]["avl_luns"]
            self.storage_info.update({
                "storage_type": "fcp",
                "boot_lun": boot_lun,
                "avl_luns": avl_luns})
        else:
            boot_lun = MACHINE_INFO[self._beaker_name]["scsi"]["boot_lun"]
            avl_luns = MACHINE_INFO[self._beaker_name]["scsi"]["avl_luns"]
            lun_addr = MACHINE_INFO[self._beaker_name]["scsi"]["lun_address"]
            lun_port = MACHINE_INFO[self._beaker_name]["scsi"]["lun_port"]
            lun_target = MACHINE_INFO[self._beaker_name]["scsi"]["lun_target"]
            self.storage_info.update({
                "storage_type": "iscsi",
                "boot_lun": boot_lun,
                "avl_luns": avl_luns,
    ...
    def _get_vm_info(self):
        ...
        self.vm_info.update({"vm_name": vm_name})
    ...
    def _get_disk_info(self):
        ...
        if disk0_type == "localfs" or disk0_type == "nfs":
            ...
            if disk0_type == "fcp":
                ...
                lun_port = ""
                lun_target = ""
                lun_id = MACHINE_INFO[self._beaker_name]["fc"]["avl_luns"][1]
                second_lun_id = MACHINE_INFO[self._beaker_name]["fc"]["avl_luns"][2]
            else:  # This is "iscsi" type
                lun_id = MACHINE_INFO[self._beaker_name]["scsi"]["avl_luns"][1]
                lun_addr = MACHINE_INFO[self._beaker_name]["scsi"]["lun_address"]
                lun_port = MACHINE_INFO[self._beaker_name]["scsi"]["lun_port"]
                lun_target = MACHINE_INFO[self._beaker_name]["scsi"]["lun_target"]
                second_lun_id = MACHINE_INFO[self._beaker_name]["scsi"]["avl_luns"][2]
            self.disk_info["disk0"].update({"lun_id": lun_id})
            self.disk_info["disk0"].update({"lun_addr": lun_addr})
            self.disk_info["disk0"].update({"lun_port": lun_port})
            self.disk_info["disk0"].update({"lun_target": lun_target})
            self.disk_info["disk1"].update({"lun_id": second_lun_id})
            self.disk_info["disk1"].update({"lun_addr": lun_addr})
            self.disk_info["disk1"].update({"lun_port": lun_port})
